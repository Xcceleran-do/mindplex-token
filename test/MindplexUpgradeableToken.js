...
describe("Mindplex Token Contract", function () {
    ...
    before(async function () {
        ...
        const _mindplexToken  = await ethers.getContractFactory("MindplexUpgradeableToken");
        mindplexToken = await upgrades.deployProxy(
            _mindplexToken, 
            [_name, _symbol]
        );
        await mindplexToken.connect(admin).deployed();
    ...
    it("should revert if trying to call initializer again", async function () { 
        await expect(mindplexToken.initialize(_name, _symbol))
    ...
    it("Should revert if exceed allowance burn from amount", async function (){
        await expect(mindplexToken.burnFrom(admin.address, 500))
    ...
    it("Should pause & unpause contract", async function (){
        ...
        // Trying to call burn when contract is paused
        await expect(mindplexToken.burn(100))
        .to.be.revertedWith("Pausable: paused");
